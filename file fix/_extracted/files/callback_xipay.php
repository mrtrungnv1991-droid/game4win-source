...
if ($verify_result) { // Nếu xác thực thành công
    ...
    if ($trade_status == 'TRADE_SUCCESS') {
        $row = $CMSNT->get_row_safe(" SELECT * FROM `payment_xipay` WHERE `out_trade_no` = ? AND `transaction_id` IS NULL AND `status` =  0 ", [$out_trade_no]);
        if ($row) {
            // Khoá dòng này TRƯỚC khi cộng tiền bằng UPDATE có điều kiện `status = 0`.
            // Nếu webhook bị gọi 2 lần cùng lúc, chỉ 1 request "thắng" được UPDATE này
            // (affected_rows = 1), request còn lại affected_rows = 0 và sẽ không cộng tiền.
            // Cách cũ (SELECT rồi mới UPDATE sau khi cộng tiền) có race condition: cả 2
            // request đều có thể đọc thấy status = 0 cùng lúc và cộng tiền 2 lần.
            $claimed = $CMSNT->update('payment_xipay', [
                'status'            => 1,
                'type'              => $type,
