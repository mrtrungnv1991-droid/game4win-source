    ...
    try {
        ...
        if ($refundType == 'partial') {
            ...
            if ($CMSNT->site('cong_tien_nguoi_ban') == 1) {
                ...
                // Dùng hash nội dung giao dịch (amount + số lượng) thay vì time() làm hậu tố
                // transid: double-click gửi trùng request sẽ ra CÙNG transid -> bị chặn trùng
                // (đúng ý muốn), còn lần hoàn tiền một phần KHÁC sau đó (số lượng/tiền khác)
                // vẫn có transid khác -> vẫn xử lý bình thường.
                $refundOpHash = substr(md5($amountRefund . '_' . $partialQuantity), 0, 12);
                $removeSeller = $User->RemoveCredits($product_order['seller'], $amountRefund, __('[Admin] Thu hồi hoàn tiền một phần đơn hàng') . ' #' . $product_order['trans_id'], 'TAKE_REFUND_partial_ORDER_' . $product_order['trans_id'] . '_' . $refundOpHash);
                if (!$removeSeller) {
            ...
            $refundOpHash = substr(md5($amountRefund . '_' . $partialQuantity), 0, 12);
            $isRefund = $User->RefundCredits($product_order['buyer'], $amountRefund, "[Admin] $reason", 'REFUND_partial_ORDER_' . $product_order['trans_id'] . '_' . $refundOpHash);
            if (!$isRefund) {
            ...
            if ($CMSNT->site('cong_tien_nguoi_ban') == 1) {
                ...
                $refundOpHash = substr(md5($amountRefund . '_' . $percentValue), 0, 12);
                $removeSeller = $User->RemoveCredits($product_order['seller'], $amountRefund, __('[Admin] Thu hồi hoàn tiền theo % đơn hàng') . ' #' . $product_order['trans_id'], 'TAKE_REFUND_percent_ORDER_' . $product_order['trans_id'] . '_' . $refundOpHash);
                if (!$removeSeller) {
                    throw new Exception(__('Không thể thu hồi tiền người bán'));
                }
            }
            ...
            // 3. Hoàn tiền cho người mua SAU
            $refundOpHash = substr(md5($amountRefund . '_' . $percentValue), 0, 12);
            $isRefund = $User->RefundCredits($product_order['buyer'], $amountRefund, "[Admin] $reason", 'REFUND_percent_ORDER_' . $product_order['trans_id'] . '_' . $refundOpHash);
            if (!$isRefund) {
                throw new Exception(__('Không thể hoàn tiền cho người mua'));
            }
