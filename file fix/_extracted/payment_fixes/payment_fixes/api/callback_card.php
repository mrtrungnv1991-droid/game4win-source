...
if(isset($_GET['request_id']) && isset($_GET['callback_sign'])){
    ...
    if($status == 1){
        ...
        // Chỉ UPDATE khi status vẫn còn 'pending' -> atomic claim, tránh callback bị
        // gọi 2 lần cùng lúc cộng tiền 2 lần cho cùng 1 thẻ.
        $claimed = $CMSNT->update("cards", array(
            'status'        => 'completed',
            'price'         => $price,
            'update_date'    => gettime()
