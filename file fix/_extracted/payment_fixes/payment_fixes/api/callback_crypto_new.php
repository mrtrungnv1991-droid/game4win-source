...
switch ($status) {
    ...
    case 'expired':
        ...
        break;
    case 'completed':
        // Giao dịch đã hoàn tất. Chỉ UPDATE khi status hiện tại KHÁC 'completed'
        // (atomic claim) để 2 callback đến cùng lúc không cùng cộng tiền.
        $isUpdate = $CMSNT->update('payment_crypto', [
            'status'            => 'completed',
            'update_gettime'    => gettime()
        ], " `id` = ? AND `status` != 'completed' ", [$row['id']]);
        if($isUpdate){
            $User = new users();
