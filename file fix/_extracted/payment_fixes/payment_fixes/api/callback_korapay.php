    ...
    if (!$verification || !isset($verification['status']) || $verification['status'] !== true) {
        ...
        if($verification['data']['status'] == 'success'){
            if($row = $CMSNT->get_row_safe(" SELECT * FROM `payment_korapay` WHERE `trans_id` = ? AND `status` =  0 ", [$reference])){
                // Khoá đơn bằng UPDATE có điều kiện status=0 TRƯỚC khi cộng tiền,
                // tránh 2 webhook gọi cùng lúc cộng tiền 2 lần (race condition).
                $claimed = $CMSNT->update('payment_korapay', [
                    'status'            => 1,
                    'updated_at'        => gettime()
