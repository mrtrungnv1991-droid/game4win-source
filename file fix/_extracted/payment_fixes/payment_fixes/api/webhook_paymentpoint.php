...
if ($notificationStatus === 'payment_successful' && $transactionStatus === 'success') {
    ...
    if ($paymentRecord) {
        ...
        // thay cho kiểm tra duplicate riêng lẻ (vốn có khoảng hở race condition).
        $claimed = $CMSNT->update('payment_paymentpoint', [
            'status' => 1,
            'amount' => $amountPaid,
        ...
        if ($isCong) {
            ...
            // Cộng tiền thất bại -> trả đơn về pending để webhook lần sau xử lý lại
