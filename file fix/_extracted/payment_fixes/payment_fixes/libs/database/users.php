...
class users extends DB
{
    ...
    public function AddCredits($user_id, $amount, $reason, $transid = NULL){
        if($transid == NULL){
            ...
            // Chống double-credit khi webhook thanh toán bị gọi lại (rất phổ biến ở các
            // cổng thanh toán) hoặc 2 request đến gần như đồng thời.
            // Lưu ý quan trọng: để chặn TRIỆT ĐỂ race condition (2 request đến cùng lúc),
            // cột `dongtien`.`transid` BẮT BUỘC phải có UNIQUE INDEX ở database.
            // Check dưới đây xử lý được phần lớn trường hợp thực tế (webhook gọi lại sau
            // vài giây/phút), còn phần chống race condition tuyệt đối nằm ở catch lỗi
            // duplicate-key khi insert bên dưới.
            $existed = parent::get_row_safe("SELECT `id` FROM `dongtien` WHERE `transid` = ?", [$transid]);
            if ($existed) {
                return false; // Giao dịch đã được xử lý trước đó, bỏ qua để tránh cộng tiền trùng
            }
        }
