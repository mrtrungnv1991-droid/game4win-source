<?php
    ...
    /* START CHỐNG SPAM */
    $elapsed = time() - (int)$CMSNT->site('time_cron_checklive_hotmail');
    ...
    if($CMSNT->site('api_check_live_hotmail') == ''){
...
$thirty_minutes_ago = time() - $CMSNT->site('time_limit_check_live_hotmail'); 
...
foreach ($uids as $uid) {
    $ch = curl_init();
    $url = $CMSNT->site('api_check_live_hotmail')."?account=".$uid."&api_key=".$CMSNT->site('api_key_check_live_hotmail');
